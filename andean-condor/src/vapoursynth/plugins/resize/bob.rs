...
impl PluginFunction for Bob {
    const FUNCTION_NAME: &'static str = "Bob";
    const FUNCTION_DOCS: Option<&'static str> =
        Some("https://www.vapoursynth.com/doc/functions/video/resize.html#resize.Bob");
...
impl Bob {
    ...
    pub fn invoke<'core>(
        ...
        Self::arguments_set(&mut arguments, vec![
            ...
            ("filter", self.filter.as_ref().map(|s| s.as_bytes())),
        ])?;
...
impl VapourSynthPluginScript for Bob {
    #[inline]
    fn generate_script(&self, node_name: NodeVariableName) -> Result<(Option<Imports>, Vec<Line>)> {
        ...
        let line = {
            ...
            write!(&mut line, "core.resize.Bob(clip = {}", node_name)?;
