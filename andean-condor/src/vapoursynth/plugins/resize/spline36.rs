...
impl Plugin for Spline36 {
    const PLUGIN_NAME: &'static str = NAME;
    const PLUGIN_ID: &'static str = ID;
    const PLUGIN_DOCS: Option<&'static str> = Some(DOCS);
}

impl PluginFunction for Spline36 {
    const FUNCTION_NAME: &'static str = "Spline36";
    const FUNCTION_DOCS: Option<&'static str> =
        Some("https://www.vapoursynth.com/doc/functions/video/resize.html#resize.Spline36");
...
impl VapourSynthPluginScript for Spline36 {
    #[inline]
    fn generate_script(&self, node_name: NodeVariableName) -> Result<(Option<Imports>, Vec<Line>)> {
        ...
        let line = {
            ...
            write!(&mut line, "core.resize.Spline36(clip = {}", node_name)?;
